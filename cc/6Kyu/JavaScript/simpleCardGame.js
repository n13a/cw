...
function winner(deckSteve, deckJosh) {
  let steveScore = 0, joshScore = 0
  const value = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A']
  for (let i = 0; deckSteve.length; ++i) {
    const steveCur = deckSteve.shift(),
      joshCur = deckJosh.shift()
    if (value.indexOf(steveCur) > value.indexOf(joshCur)) steveScore++
    else if (value.indexOf(steveCur) < value.indexOf(joshCur)) joshScore++
  }
